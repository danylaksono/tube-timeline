...
export class TubeTimeline {
  constructor(cfg) {
    ...
    if (!this.d3)
      throw new Error(
        "TubeTimeline: d3 v7 is required (pass via cfg.d3 or include globally)"
      );
    this.root =
      typeof target === "string" ? document.querySelector(target) : target;
    if (!(this.root instanceof SVGElement))
      throw new Error("TubeTimeline: target must be an <svg> element");
    ...
    this.options = {
      ...
      onMilestoneClick:
        options.onMilestoneClick ||
        ((m) => m.url && window.open(m.url, "_blank")),
      onMilestoneDrag: options.onMilestoneDrag || null,
      // When true, tracks are sorted by their earliest milestone date.
      // Set to false to preserve the order supplied in `data`.
      sortTracksByStartDate:
        options.sortTracksByStartDate === undefined
          ? true
          : Boolean(options.sortTracksByStartDate),
  ...
  preprocess() {
    this.data.forEach((track) =>
      track.dates.forEach((d) => (d.parsed = this.parseDate(d.date)))
    );
    // Optionally sort tracks by their earliest milestone date. If disabled,
    // the order provided in `data` will be preserved (useful when a UI
    // exposes reordering controls).
    if (this.options.sortTracksByStartDate) {
      this.data.sort((a, b) => {
        const aStart =
          a.dates.find((d) => d.type === "start" && d.parsed)?.parsed ||
          a.dates.find((d) => d.parsed)?.parsed;
        const bStart =
          b.dates.find((d) => d.type === "start" && d.parsed)?.parsed ||
          b.dates.find((d) => d.parsed)?.parsed;
        return (
          (aStart || new Date(9999, 0, 1)) - (bStart || new Date(9999, 0, 1))
        );
      });
    }
    this.allDates = [
      ...new Set(
        this.data.flatMap((t) => t.dates.map((d) => d.parsed).filter(Boolean))
      ),
    ].sort((a, b) => a - b);
  ...
  renderHeader(isHorizontal, scale) {
    ...
    if (header.logoHref) {
      headerGroup
        ...
        .attr("href", header.logoHref)
        .attr("preserveAspectRatio", "xMidYMid meet");
    }
    headerGroup
      .append("text")
      .attr("x", (header.logoHref ? logoHeight : 0) + 12)
      .attr("y", logoHeight * 0.6)
    ...
    this.data.forEach((track, i) => {
      ...
      const tempText = headerGroup
        ...
        .attr("x", 0)
        .attr("y", -100)
        .attr("font-size", isHeightConstrained ? 10 : 13)
        .attr("font-family", "sans-serif")
        .text(track.track);
    ...
    this.data.forEach((track, i) => {
      ...
      const xBase = col * colWidth,
        yBase = row * (isVertical ? 10 : legendSpacing) + (isVertical ? 2 : 4);
      ...
      itemGroup
        ...
        .attr("x1", xBase)
        .attr("y1", yBase)
        .attr("x2", xBase + (isHeightConstrained ? 8 : 12))
        .attr("y2", yBase)
        .attr("stroke", track.color)
        .attr("stroke-width", isHeightConstrained ? 4 : 6)
        .attr("stroke-linecap", "round");

      itemGroup
        .append("text")
        .attr("x", xBase + (isHeightConstrained ? 12 : 17))
        .attr("y", yBase + (isHeightConstrained ? 3 : 4))
  ...
  render() {
        ...
        if (d.label) {
          ...
          labelLines.forEach((line) => {
            if (line.trim()) {
              ...
              const rotatedExtentRight =
                textWidth * Math.cos(rotationRad) +
                textHeight * Math.sin(rotationRad);
              const rotatedExtentBottom =
                textWidth * Math.sin(rotationRad) +
                textHeight * Math.cos(rotationRad);
              maxLabelExtentRight = Math.max(
                maxLabelExtentRight,
                rotatedExtentRight
              );
              maxLabelExtentBottom = Math.max(
                maxLabelExtentBottom,
                rotatedExtentBottom
              );
            }
          });
    ...
    const requiredRightMargin = isHorizontal
      ? Math.max(
          baseRightMargin,
          baseLeftMargin / 2 + maxLabelExtentRight + safetyPadding
        )
      : baseRightMargin;
    ...
    const verticalLabelExtent =
      labelXOffset + maxLabelExtentRight + safetyPadding;
    ...
    const margin = {
      ...
      right: isHorizontal
        ? requiredRightMargin
        : Math.max(baseRightMargin, requiredRightMarginVertical),
      bottom: isHorizontal
        ? baseBottomMargin
        : Math.max(baseBottomMargin, requiredBottomMarginVertical),
      left: isHorizontal
        ? baseLeftMargin
        : Math.max(baseLeftMargin, requiredLeftMarginVertical),
    };

    this.renderHeader(isHorizontal, scale);
    const height =
      this.root.clientHeight - margin.top - margin.bottom - headerHeight;
    const g = this.svg
      .append("g")
      .attr(
        "transform",
        `translate(${isHorizontal ? margin.left / 2 : margin.left},${
          margin.top + headerHeight
        })`
      );
    const width =
      this.root.clientWidth -
      (isHorizontal ? margin.right : margin.left + 1.5 * margin.right);
    ...
    if (isHorizontal) {
      xScale = d3
        .scaleTime()
        .domain(d3.extent(this.allDates))
        .range([margin.left, width]);
      yScale = d3
        .scalePoint()
        .domain(this.trackNames)
        .range([20, height - height * 0.15])
        .padding(0.1);
      this.xScale = xScale;
      this.yScale = yScale;
      trackLinesGroup
        ...
        .attr("x1", (d) => {
          const start =
            d.dates.find((date) => date.type === "start" && date.parsed)
              ?.parsed ||
            d.dates.find((date) => date.parsed)?.parsed ||
            this.allDates[0];
          return xScale(start);
        })
        .attr("x2", (d) => {
          const end =
            d.dates.find((date) => date.type === "end" && date.parsed)
              ?.parsed ||
            d.dates
              .slice()
              .reverse()
              .find((date) => date.parsed)?.parsed ||
    ...
    this.data.forEach((track) => {
      ...
      trackLinesGroup
        .selectAll()
        .data(track.dates.filter((d) => d.parsed))
        .enter()
        .append("g")
        ...
        .each(function (d) {
          ...
          if (d.type === "start" || d.type === "end") {
            const handleRect = milestoneGroup
              .append("rect")
            ...
            if (timelineInstance.options?.onMilestoneDrag) {
              handleRect.attr("cursor", "grab").style("cursor", "grab");
              ...
              const drag = d3
                ...
                .on("end", function (event) {
                  ...
                  if (
                    draggedDate !== null &&
                    timelineInstance.options.onMilestoneDrag
                  ) {
                    const fmtDate = (date) => {
          ...
          labelLines.forEach((line, i) => {
            ...
            const labelYOffset = isHorizontal
              ? -laneHeight * 2.75 + i * labelSpacing
              : 5 + i * labelSpacing;
            ...
            d3.select(this)
              ...
              .attr("text-anchor", "start")
              .attr("font-size", isHeightConstrained ? 11 : 15)
              .attr("font-weight", "bold")
              .attr("font-family", "sans-serif")
              .attr("fill", "#222")
              .attr("x", labelXOffset)
        ...
        .on("mouseover", (event, d) => {
          ...
          this.showSvgTooltip(
            ...
            x,
            y,
            `<strong>${tooltipTitle}</strong><br>${d.date}`,
            isHorizontal
          );
        })
    ...
    if (this.allDates.length) {
      months = this.d3.timeMonths(
        ...
        this.d3.timeMonth.offset(
          this.d3.timeMonth.ceil(this.allDates[this.allDates.length - 1]),
          1
        )
      );
    }
    ...
    if (this.options.showToday && this.allDates.length) {
      ...
      if (
        today >= this.allDates[0] &&
        today <= this.allDates[this.allDates.length - 1]
      ) {
        ...
        const baseFlagWidth = Math.max(
          48,
          Math.min(80, this.root.clientWidth * 0.045)
        );
        const baseFlagHeight = Math.max(
          20,
          Math.min(32, this.root.clientHeight * 0.025)
        );
        ...
        if (isHorizontal) {
          flagGroup
            ...
            .attr(
              "points",
              `${flagX},${flagY} ${flagX + baseFlagWidth},${
                flagY + baseFlagHeight / 2
              } ${flagX},${flagY + baseFlagHeight}`
            )
            .attr("fill", "#e74c3c")
            .attr("stroke", "#b03a2e")
            .attr("stroke-width", 2);
          ...
          flagGroup
            .append("rect")
            .attr("x", fx)
            .attr("y", flagY - baseFlagHeight / 2)
            .attr("width", baseFlagWidth)
            .attr("height", baseFlagHeight)
            .attr("rx", 4)
            .attr("fill", "#e74c3c")
            .attr("stroke", "#b03a2e")
            .attr("stroke-width", 2);
